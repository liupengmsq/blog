package pengliu.me.test.dataset;
...
public class MultiSchemaXlsDataSetReader {
		...
		try {
			...
			for (Entry<String, List<ITable>> entry : tableMap.entrySet()) {
				...
				try {
					...
					throw new UnitilsException("Error when read dataset from xls",  e);
				}
			}
			return dataSets;
		} catch (Exception e) {
	...
	private Map<String, List<ITable>> getTables(File... dataSetFiles) {
		...
		try {
			...
			throw new UnitilsException("Error when get tables from data set files"
